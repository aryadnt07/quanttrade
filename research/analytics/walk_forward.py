...
1. In-Sample (IS / Year 1 / Training)
2. Out-of-Sample (OOS / Year 2 / Blind Test)
...
from src.strategies.portfolio.engine import PortfolioEngine, PortfolioStats
from src.strategies.asian_mr.signals import compute_asian_indicators
from src.strategies.asian_mr.backtester import AsianBacktester
...
COLOR_IS      = "#38bdf8"       # Cyan untuk In-Sample
COLOR_OOS     = "#fbbf24"       # Gold untuk Out-of-Sample
COLOR_TOTAL   = "#22c55e"       # Hijau Zamrud
COLOR_LOSS    = "#ef4444"       # Merah
COLOR_SPLIT   = "#ec4899"       # Pink garis pemisah
COLOR_BG      = "#0b0e14"
COLOR_PANEL   = "#111827"
COLOR_GRID    = "#1f2937"
...
def run_walk_forward_analysis(
    ...
    split_date_str: str = "2024-09-08 00:00:00+00:00",
) -> Dict[str, Any]:
    """Eksekusi Walk-Forward Analysis (IS vs OOS)."""
    ...
    # 1. Master Portfolio: Full Continuous Compounding
    print("[1/3] Menjalankan Master Portfolio Continuous (Penuh)...")
    engine_full = PortfolioEngine(df_raw)
    stats_full = engine_full.run()

    # 2. Master Portfolio: In-Sample
    print("[2/3] Menjalankan Master Portfolio In-Sample ($10k Start)...")
    engine_is = PortfolioEngine(df_is)
    stats_is = engine_is.run()

    # 3. Master Portfolio: Out-of-Sample Blind Test
    print("[3/3] Menjalankan Master Portfolio Out-of-Sample Blind Test ($10k Start)...")
...
def print_walk_forward_report(res: Dict[str, Any]) -> None:
    ...
    print(f"    - WFE Sharpe Stability     : {res['wfe_sharpe']:5.1f}%")
    print(f"    - WFE Profit Factor        : {res['wfe_pf']:5.1f}%")
...
def plot_walk_forward_dashboard(res: Dict[str, Any], save_path: str = "output/walk_forward_dashboard.png") -> None:
    ...
    # PANEL 2: Metric Comparison Grouped Bar Chart
    ax1 = axes[1]
    ax1.set_title("Performance Metrics Comparison: In-Sample vs Out-of-Sample (Blind Test)",
                  fontsize=11, fontweight="bold", color="#93c5fd", loc="left")
    ...
    bar_is = ax1.bar(x - width/2, vals_is, width, label="In-Sample (Training)", color=COLOR_IS, alpha=0.85, edgecolor="#0b0e14")
    bar_oos = ax1.bar(x + width/2, vals_oos, width, label="Out-of-Sample (Blind)", color=COLOR_OOS, alpha=0.85, edgecolor="#0b0e14")
    ...
    # PANEL 4: Comparative Equity Curve
    ax3 = axes[3]
    ...
    ax3.set_xlabel("Trade Sequence Number (Within Period)", fontsize=9, color="#9ca3af")
...
def main():
    ...
    plot_walk_forward_dashboard(res, save_path="output/walk_forward_dashboard.png")
