...
3. Live Trade Journal: Pencatatan otomatis setiap transaksi (Entry & Exit)
   ke berkas CSV bersih (logs/live_trade_journal.csv) untuk audit & analisis Excel.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# 1. WINDOWS QUICK-EDIT MODE PROTECTION
# ─────────────────────────────────────────────────────────────────────────────

def disable_quick_edit_mode() -> bool:
    """
    Mematikan QuickEdit Mode pada Windows Console.
    
    Latar Belakang Institusional:
    Di Windows CMD / PowerShell, jika jendela terminal diklik, konsol masuk ke
    mode seleksi teks dan sistem operasi MEMBEKUKAN proses Python secara total.
    Fungsi ini menggunakan Windows Win32 API (kernel32.dll) untuk menonaktifkan
    ENABLE_QUICK_EDIT_MODE (0x0040) sehingga bot aman 100% dari salah klik di VPS.
    """
    ...
    try:
        ...
        # Matikan QuickEdit Mode dan wajib sertakan flag extended
...
# ─────────────────────────────────────────────────────────────────────────────
# 2. DUAL-LOGGING ENGINE (CONSOLE + DAILY ROTATING FILE)
# ─────────────────────────────────────────────────────────────────────────────

class UTCFormatter(logging.Formatter):
    """Formatter logging yang selalu mengonversi waktu log menjadi format standar UTC."""
...
def setup_logger(
    ...
    - File Handler: Menulis ke logs/live_trading.log dengan rotasi harian setiap 00:00 UTC.
    - Stream Handler: Menulis log bersih ke CMD terminal dengan format UTC presisi.
    """
    ...
    with _LOGGER_LOCK:
        ...
        if log_dir is None:
            log_dir = getattr(lcfg, "LOG_DIR", os.path.join(os.getcwd(), "logs"))
        ...
        # 2. Console Stream Handler (Output ke Terminal CMD)
...
def get_logger() -> logging.Logger:
    """Helper untuk mengambil logger yang sudah diinisialisasi atau inisialisasi default."""
...
# ─────────────────────────────────────────────────────────────────────────────
# 3. LIVE TRADE JOURNAL CSV (AUDIT TRAIL INSTITUSIONAL)
# ─────────────────────────────────────────────────────────────────────────────
...
class TradeJournal:
    ...
    Thread-safe dan menulis header otomatis saat file baru dibuat.
    """

    def __init__(self, filepath: Optional[str] = None):
        if filepath is None:
            filepath = getattr(lcfg, "TRADE_JOURNAL_FILE", os.path.join(os.getcwd(), "logs", "live_trade_journal.csv"))
        self.filepath = filepath
        self._lock = threading.Lock()
        self._ensure_file_ready()

    def _ensure_file_ready(self):
        """Buat folder dan tulis baris header jika berkas belum ada."""
        with self._lock:
    ...
    def record_entry(
        ...
        """Catat transaksi baru yang baru saja terisi di server broker (ENTRY)."""
        ...
        row = [
            ...
            f"{volume:.2f}",
            f"{open_price:.3f}",
            "",                 # close_price kosong saat entry
            f"{sl:.3f}",
            f"{tp:.3f}",
            f"{slippage_pts:.1f}",
            "",                 # net_pnl_usd kosong saat entry
            "",                 # r_multiple kosong saat entry
            "",                 # exit_reason kosong saat entry
            comment
        ]
    ...
    def record_exit(
        ...
        """Catat hasil transaksi yang telah ditutup (EXIT)."""
        if timestamp_utc is None:
            timestamp_utc = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
