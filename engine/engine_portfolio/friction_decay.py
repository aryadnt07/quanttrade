...
3. Mengidentifikasi metrik kunci institusional:
   - Break-Even Friction Threshold (F_BE): Titik impas di mana Net PnL = $0 / PF = 1.0
   - Safety Buffer Multiplier: Kelipatan spread normal (0.30 USD) yang dapat ditoleransi
   - Decay Half-Life (F_50%): Friction di mana profit terpangkas 50%
   - Sub-strategy Fragility: Perbandingan sensitivitas Asian MR vs NY ORB
4. Menghasilkan visual dashboard 4-panel institusional (output/friction_decay_dashboard.png).
"""
...
def extract_raw_trades(data_path: str = "data/xauusd-m5-bid-2024-09-08-2026-09-08.csv") -> Tuple[List[Dict], List[Dict], List[Dict]]:
    ...
    # Combined master list sorted chronologically
...
def simulate_portfolio_under_friction(
    ...
    for t in trades:
        ...
        # PnL setelah pengurangan friction (Spread + Slippage)
...
def run_friction_decay_sweep(
    ...
    # Menghitung titik Break-Even (F_BE) via interpolasi linear
    ...
    # Menghitung Half-Life Decay (F_50%)
...
def print_friction_report(res: Dict[str, Any]) -> None:
    ...
    print(f"  - Sistem mampu menyerap lonjakan spread/slippage hingga {buffer_ratio:.1f}x lipat dari kondisi normal")
    print("    sebelum mencapai titik impas (break-even zero profit).")
    print("  - Pada kondisi spread ekstrim ($0.75 USD/oz atau 2.5x lipat), Master Portofolio tetap mencetak")
    print(f"    ROI +{m_res[7]['roi_pct']:.1f}% (Net PnL +${m_res[7]['net_pnl']:,.2f} USD) dengan Profit Factor {m_res[7]['profit_factor']:.2f}.")
    print("  - NY ORB memiliki toleransi friction lebih tinggi karena target ekspansi momentum yang jauh lebih lebar,")
    print("    sementara Asian MR menyediakan kestabilan frekuensi pada kondisi normal.")
    print("=" * 90 + "\n")
...
def plot_friction_decay_dashboard(
    ...
    """Render 4-panel visual dashboard grafik degradasi friction tanpa penomoran."""
    f_levels = res["friction_levels"]
    m_res = res["master_results"]
    a_res = res["asian_results"]
    ny_res = res["ny_results"]
    ...
    # ── PANEL 1: Ending Capital & Equity Decay Curve ($ USD) ──
    ax1 = axes[0, 0]
    ...
    # ── PANEL 2: Profit Factor Decay Surface (Master vs Sub-Strategies) ──
    ax2 = axes[0, 1]
    ...
    # Highlight point at baseline and break-even
    ...
    ax3.annotate(f"Baseline: {wr_base:.1f}%",
                 xy=(base_spread, wr_base),
                 xytext=(base_spread + 0.15, wr_base + 3),
    ...
    ax4.annotate(f"Baseline DD: {dd_base:.1f}%",
                 xy=(base_spread, dd_base),
                 xytext=(base_spread + 0.15, dd_base - 5),
                 fontsize=8.5, fontweight="bold", color=COLOR_GOLD,
                 arrowprops=dict(arrowstyle="->", color=COLOR_GOLD, lw=1.2))
