...
def calc_session_vwap(df: pd.DataFrame, session_labels: pd.Series) -> pd.Series:
    ...
    Parameters
    ----------
    df : DataFrame dengan kolom 'close' dan 'volume'
    session_labels : Series berisi label unik per sesi (misal tanggal)
    
    Returns
    -------
    pd.Series : VWAP per bar
    """
...
def compute_all(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Input DataFrame harus memiliki kolom: datetime, open, high, low, close
    (opsional: volume).
    
    Returns DataFrame dengan kolom tambahan:
        sma, std, zscore, rsi, atr, atr_sma, upper_band, lower_band, htf_ema
    """
    df = df.copy()
