...
class Backtester:
    ...
    Usage:
        bt = Backtester(df_with_indicators)
        bt.run()
        print(bt.stats)
        bt.trade_log  # DataFrame semua trade
    """
    ...
    def __init__(self, df: pd.DataFrame):
        ...
        Parameters
        ----------
        df : DataFrame lengkap dengan kolom indikator dari indicators.compute_all()
        """
        self.df = df.copy()
    ...
    def run(self) -> BacktestStats:
        ...
        for idx, row in self.df.iterrows():
            ...
            if self._current_signal is not None:
                ...
                continue  # Jangan buka posisi baru di bar yang sama
    ...
    def _update_mfe_mae(self, row: pd.Series) -> None:
        """Update Maximum Favorable/Adverse Excursion untuk trade berjalan."""
    ...
    def _open_trade(self, signal: Signal) -> None:
        ...
        self.detailed_log_lines.append(f"    -> Targets   : TP = {signal.take_profit:.2f} | SL = {signal.stop_loss:.2f}")
        self.detailed_log_lines.append(f"")
    def _close_trade(self, row: pd.Series, idx: int, reason: ExitReason) -> None:
        """Tutup posisi dan catat hasilnya."""
        signal = self._current_signal
        if signal is None:
            return
        ...
        self.detailed_log_lines.append(f"--- TRADE EXIT ---")
    ...
    def _calc_unrealized_pnl(self, row: pd.Series) -> float:
        """Hitung PnL yang belum terealisasi untuk equity curve."""
    ...
    def _compute_stats(self) -> BacktestStats:
        """Hitung statistik ringkasan dari semua trade."""
        stats = BacktestStats()
        ...
        if len(pnls) > 1:
            ...
            if std_ret > 0:
                # Rata-rata trades per hari di sesi Asia ≈ 2-3
        ...
        for t in self.trades:
            if t.exit_reason == ExitReason.TAKE_PROFIT:
                ...
                stats.exits_sl += 1
            elif t.exit_reason == ExitReason.HARD_CUT:
                stats.exits_hard_cut += 1
            elif t.exit_reason == ExitReason.TIME_STOP:
    ...
    def _trading_days(self) -> int:
        """Hitung jumlah hari trading unik dalam data."""
        if self.df.empty:
            return 1
    ...
    def trade_log(self) -> pd.DataFrame:
        """Konversi daftar trade ke DataFrame untuk analisis."""
        if not self.trades:
            return pd.DataFrame()
