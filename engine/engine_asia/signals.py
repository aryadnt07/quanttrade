...
def check_entry_signal(row: pd.Series, idx: int) -> Optional[Signal]:
    ...
    # Filter Tren: Batalkan jika pasar sedang trending (ADX tinggi)
    # if "adx" in row and pd.notna(row["adx"]) and row["adx"] >= cfg.ADX_MAX_THRESHOLD:
    #     return None
...
def check_exit_conditions(
    ...
    # 5. Session Cutoff (Market Tutup / Hard Kill Switch)
    # Jika bar saat ini adalah bar terakhir di sesi tersebut, exit
    ...
    if current_minutes >= end_minutes - 5:  # 5 menit sebelum session hard cutoff
        return ExitReason.SESSION_CUTOFF
    ...
    # ── 3. HARD CUT (|Z| > 3.2) — deviasi makin melebar ──
    ...
    # ── 4. STOP LOSS (harga menyentuh SL) ──
    ...
    # ── 5. TAKE PROFIT (Z kembali ke area ekuilibrium) ──
...
def get_exit_price(
    ...
    # Untuk session cutoff, time stop, dan hard cut — exit di close bar saat ini
    return row["close"]
