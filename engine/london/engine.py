"""
London Pit Opening Range Breakout (London Pit ORB) — Execution Engine & Backtester
===================================================================================
Mesin backtest event-driven sesuai spesifikasi kuantitatif:
- Pembentukan Opening Range M15 (08:00 - 08:15 UTC)
- Evaluasi sinyal pada jendela 08:15 - 11:30 UTC
...
from engine.london.strategy import LondonStrategy, LondonSignal, Direction, compute_london_indicators
from engine.london.trade_manager import LondonTradeManager, LondonTrade, ExitReason


@dataclass
class LondonStats:
    """Statistik performa strategi London Pit ORB."""
    ...
    def __str__(self) -> str:
        ...
        return (
            f"\n{sep}\n"
            f"   LONDON PIT OPENING RANGE BREAKOUT (LONDON ORB) — REPORT\n"
            f"{sep}\n"
...
class LondonBacktester:
    """Mesin simulasi event-driven London Pit ORB."""
    ...
    def __init__(self, df: pd.DataFrame, initial_capital: float = cfg.INITIAL_CAPITAL, df_m1: Optional[pd.DataFrame] = None):
        ...
        self.strategy = LondonStrategy()
        self.trade_manager = LondonTradeManager(current_capital=initial_capital)

        self.equity_curve: List[float] = [initial_capital]
        self.equity_dates: List[pd.Timestamp] = []
        self.stats: LondonStats = LondonStats(initial_capital=initial_capital)

    def run(self) -> LondonStats:
        """Eksekusi simulasi London Pit ORB (M1 Bar Magnifier jika df_m1 tersedia, else M5)."""
        if self.df_m1 is not None:
            return self._run_m1()
        return self._run_m5()

    def _run_m1(self) -> LondonStats:
        """Eksekusi presisi sub-bar M1 (Bar Magnifier) dengan setup sinyal M5."""
        if "lor_high" not in self.df_raw.columns or "tr_sma20" not in self.df_raw.columns:
            df_m5 = compute_london_indicators(self.df_raw)
        else:
            df_m5 = self.df_raw
        ...
        for date, day_m1 in m1.groupby("date"):
            trade_count_today = 0
            lor_bars = day_m1[(day_m1["hour"] == 8) & (day_m1["minute"] >= 0) & (day_m1["minute"] < 15)]
            if len(lor_bars) < cfg.MIN_LORB_BARS:
                continue

            lor_high = lor_bars["high"].max()
            lor_low = lor_bars["low"].min()
            lor_range = lor_high - lor_low
            if lor_range <= 0:
                continue

            window = day_m1[((day_m1["hour"] == 8) & (day_m1["minute"] >= 15)) |
                            ((day_m1["hour"] > 8) & (day_m1["hour"] < 11)) |
                            ((day_m1["hour"] == 11) & (day_m1["minute"] < 30))].copy()
            if window.empty:
                continue
    ...
    @property
    def trades(self) -> List[LondonTrade]:
    ...
    def _run_m5(self) -> LondonStats:
        """Eksekusi simulasi London Pit ORB pada dataset M5."""
        if "lor_high" not in self.df_raw.columns or "tr_sma20" not in self.df_raw.columns:
            df = compute_london_indicators(self.df_raw)
        else:
            df = self.df_raw
        ...
        for date, day_df in df.groupby("date"):
            ...
            lor_df = day_df[(day_df["hour"] == 8) & (day_df["minute"] >= 0) & (day_df["minute"] < 15)]
            if len(lor_df) < cfg.MIN_LORB_BARS:
                continue

            lor_high = lor_df["high"].max()
            lor_low = lor_df["low"].min()
            lor_range = lor_high - lor_low
            if lor_range <= 0:
                continue

            window = day_df[((day_df["hour"] == 8) & (day_df["minute"] >= 15)) |
                            ((day_df["hour"] > 8) & (day_df["hour"] < 11)) |
                            ((day_df["hour"] == 11) & (day_df["minute"] < 30))].copy()
